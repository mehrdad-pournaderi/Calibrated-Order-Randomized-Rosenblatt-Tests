...
keys = ["single", "pmerge", "bonf", "eavg", "chi2", "sym", "fisher"]

fig, ax = plt.subplots(1, 3, figsize=(13, 4.1))

A = ax[0]
for k in keys:
    A.plot(x, d[f"{k}_ns"], "-o", color=C[k], lw=1.7, ms=3.5, label=LBL[k])
...
B = ax[1]
for k in keys:
    B.plot(x, d[f"{k}_cs"], "-o", color=C[k], lw=1.7, ms=3.5, label=LBL[k])
...
Cx = ax[2]
for k in keys:
    Cx.plot(x, d[f"{k}_cp"], "-o", color=C[k], lw=1.7, ms=3.5, label=LBL[k])
...
Cx.legend(fontsize=6.8, loc="lower right")
